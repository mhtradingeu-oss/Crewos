...
describe("pricing validators", () => {
  ...
  it("competitorPriceCreateSchema: throws on missing competitor", () => {
    expect(() => competitorPriceCreateSchema.parse({ priceNet: 10, priceGross: 12 })).toThrow();
  });

  it("validates pricingListQuerySchema: valid", () => {
    expect(pricingListQuerySchema.parse({ page: 1, pageSize: 10 })).toMatchObject({ page: 1, pageSize: 10 });
  });
  it("pricingListQuerySchema: throws on negative page", () => {
    expect(() => pricingListQuerySchema.parse({ page: 0 })).toThrow();
  });
});
